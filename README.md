# THM-Writeups – TryHackMe Writeups

Étudiant en cybersécurité au Bénin | Focus pentest & audits | Freelance

**Objectif :** Pentester/Red Teamer en freelance  
**Contact :** norbertaziamadjidev@gmail.com

## Roadmap
- **Pre-Security :** Terminé (modules 1- 5, writeup module 5 en production)
- **Cybersecurity 101 :** En cours (renforcement concepts Pre-Security + avancés)
- **Junior Penetration Tester (Red Team) :** Prochain path 2026

## Writeups
### Pre-Security Path
- [Introduction à la cybersécurité](writeups/pre-security/intro-cybersecurity.md)
- [Les fondamentaux du réseau](writeups/pre-security/network-fundamentals.md)
- [Comment fonctionne le web](writeups/pre-security/how-the-web-works.md)
- [Les fondamentaux de Linux](writeups/pre-security/linux-fundamentals.md)

## Screenshots & Preuves
Voir [dossier screenshots](screenshots/)

## À venir
- Cheatsheets communes (nmap, burp, linux commands)
- Templates writeups standardisés

---

**Note :** J'utilise actuellement un compte TryHackMe gratuit. Ma progression dans ce parcours est donc limitée aux salles accessibles sans abonnement.

Feedback welcome – je construis mon portfolio freelance cybersécurité.

**Dernière mise à jour :** 28/12/2025

## **Statut : "Repo en construction depuis le 24 déc. 2025"**